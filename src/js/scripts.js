...
function getPosts(){
    let d = document;
    let feedBlog = d.getElementById("feed-blog");
    var url = 'php/feed-wd.php';
    // primeiro identifica se existe o feed-blog
    if(feedBlog){
        var nPost = '?n='+feedBlog.getAttribute('data-n-post');
        let urlGet = url+nPost;
        console.log(urlGet);
        $.ajax({
            url: urlGet,
            type: 'GET',
